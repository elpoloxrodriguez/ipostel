...
public DolarPetroDia = []
  ...
  CertificadoInscripcion(data: any, Qr: any, TokenQr: any, n_curp: any) {
    ...
    // console.log(nueva)
    // console.log(data.periodo_contrato_curp)
  ...
  AutorizacionInscripcion(data: any, Qr: any, TokenQr: any,  n_curp: any) {
    ...
    doc.setFont(undefined, "");
    doc.setFontSize(12);
    doc.text(`    Conforme a lo aprobado por el Directorio del Instituto Postal Telegráfico de Venezuela, según Punto de Cuenta N° ${ data.punto_cuenta_curp } en fecha ${ data.fecha_punto_cuenta_curp }, el monto a cancelar por la empresa ${ data.nombreOpp.toUpperCase() }, por concepto de obligaciones postales, es el equivalente al cincuenta por ciento(50 %) del costo actual de la Concesión Postal, para lo cual deberá cancelar en el lapso que corresponda al Operador Postal Privado a que está supeditado, conforme a lo establecido en la Contrato de Concesión suscrito por su representada.`,
      14,
      215,
      { maxWidth: 185, align: "justify" }
    );
   ...
   GenerarFactura(data: any,MantenimientoYSeguridad: any){
    ...
    this.tempDataRowsFacturas = [
      ...
      for (let i = 0; i < MantenimientoYSeguridad.length; i++) {
        ...
        // console.log(element)
        var valor0  = element[0].tasa_petro * data[0].ListaFacturas[0].petro_dia
        this.tempMantenimiento = [
          element[0].nombre_tipo_pagos,
          element[0].iniciales_tipo_pagos,
          this.utilService.ConvertirMoneda(valor0),
          '---',
          '---',
        ];
        var valor1  = element[1].tasa_petro * data[0].ListaFacturas[0].petro_dia
        this.tempSeguridad = [
          element[1].nombre_tipo_pagos,
          element[1].iniciales_tipo_pagos,
          this.utilService.ConvertirMoneda(valor1),
          '---',
          '---',
        ];
      }
    ...
    autoTable(doc, {
      ...
      let montoPetro = total  / parseFloat(data[0].ListaFacturas[0].petro_dia)
      autoTable(doc,{
        ...
        body: [['TOTAL A CANCELAR', this.TotalPagarFactura ? this.TotalPagarFactura : 0, 'MONTO PETRO', montoPetro.toFixed(8) ? montoPetro.toFixed(8) : 0  ]],
        theme: "grid",
        startY: 98,
      })
